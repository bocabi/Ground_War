...
void Stage2::Init()
{
	...
	te = text;
	GameEnd = true;
	OBJ->AddObj(new Player(2), 1);
	OBJ->AddObj(new Stage2Boss(), 2);
...
void Stage2::Render()
{
	...
	if (GameOver)
	{
		...
		else
		{
			...
			IMAGE->DarwText(a, Vec2(OBJ->player->Pos.x - FirstPos.x, OBJ->player->Pos.y + FirstPos.y), Vec2(3, 3), D3DCOLOR_XRGB(255, 0, 0));
		}
	}
	...
	{
		ALPHA -= 2;
		IMAGE->DarwText("Stage2", Vec2(OBJ->player->Pos.x - FirstPos.x, OBJ->player->Pos.y + FirstPos.y), Vec2(4, 4), D3DCOLOR_XRGB(255, 0, 0));
	}
}
